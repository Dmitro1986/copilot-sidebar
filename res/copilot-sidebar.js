...
module.exports = function(RED) {
    ...
    class CopilotAnalyzer {
        ...
        // Генерация предложений для потока
        generateFlowSuggestions(flow) {
            const suggestions = [];
            
            // Базовые предложения на основе типов узлов
            const nodeTypes = flow.nodes.map(n => n.type);
            
            if (nodeTypes.includes('http in') && !nodeTypes.includes('catch')) {
                suggestions.push('Добавьте catch узел для обработки ошибок HTTP запросов');
            }
            
            if (nodeTypes.includes('function') && !nodeTypes.includes('debug')) {
                suggestions.push('Добавьте debug узлы для отладки function узлов');
            }
            
            return suggestions;
        }

        // Анализ производительности потока
        analyzeFlowPerformance(flow) {
            const performance = {
                score: 100,
                issues: []
            };
            ...
            if (httpNodes.length > 3) {
                ...
                performance.issues.push('Много HTTP запросов может замедлить выполнение');
            }
            
            const functionNodes = flow.nodes.filter(n => n.type === 'function');
            if (functionNodes.length > 5) {
                performance.score -= 10;
                performance.issues.push('Много function узлов может усложнить отладку');
            }
            
            return performance;
        }

        // Генерация глобальных предложений
        generateGlobalSuggestions(flows) {
            const suggestions = [];
            
            if (flows.length > 5) {
                suggestions.push('Рассмотрите использование subflow для переиспользования логики');
            }
            
            const totalNodes = flows.reduce((sum, flow) => sum + (flow.nodes ? flow.nodes.length : 0), 0);
            if (totalNodes > 50) {
                suggestions.push('Большое количество узлов - рассмотрите оптимизацию');
            }
    ...
    RED.httpAdmin.get('/copilot-sidebar/analyze', async function(req, res) {
        try {
            ...
            // Пока используем только стандартный анализ (AI будет работать через браузерные классы)
    ...
    // API для управления моделями (простая версия без зависимостей)
    RED.httpAdmin.get('/copilot-sidebar/models', function(req, res) {
        try {
            // Возвращаем базовый список моделей
            const models = [
                {
                    id: 'builtin-analyzer',
            ...
            res.json({
                models,
                currentModel: { id: 'builtin-analyzer', name: 'Встроенный анализатор' },
    ...
    RED.httpAdmin.post('/copilot-sidebar/models/current', function(req, res) {
        try {
            ...
            // Простая заглушка - в реальности здесь будет сохранение в localStorage через браузер
            res.json({ 
                success: true, 
                currentModel: { id: modelId, name: 'Модель изменена' }
            });
        } catch (err) {
            res.status(500).json({ error: err.message });
        }
